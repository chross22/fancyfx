...
effects via `gratia`, and everything else as predictions via `marginaleffects`.

> **Renamed from `fancygam`.** The package used to handle only GAMs. `plotSmooths()`
> still works and produces an identical plot, but is deprecated in favour of
> `plotEffects()` — same arguments, same output. See
> [Migrating](#migrating-from-fancygam).
...
Support beyond those comes from whatever `marginaleffects` handles, and is
being verified model family by model family — Bayesian fits next.
